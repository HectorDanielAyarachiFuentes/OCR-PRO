  ...
  workerCreationPromise = (async () => {
    ...
    while (attempts < 2) {
      try {
        ...
        const corePath = chrome.runtime.getURL('OCR/lib/tesseract-core-simd.wasm.js');
        const langPath = chrome.runtime.getURL('OCR/lib/langdata/');
      ...
      try {
        ...
        if (forceTableFormat) {
          ...
          for (const row of mergedLines) {
            ...
            const cleaned = cleanAndFormatWords(row.words, preprocessed.origColors, preprocessed.imgWidth);
          ...
          for (const block of blocks) {
            for (const para of block.paragraphs || []) {
              for (const line of para.lines || []) {
                const cleanedLine = cleanAndFormatWords(line.words, preprocessed.origColors, preprocessed.imgWidth);
                if (cleanedLine) {
        ...
        let formattedText = finalOutput.trim().split('\n').map(line => {
          ...
          // Normalizar viñetas de listas con basura OCR (emojis mal reconocidos)
          // Ejemplos: "++ Características", "¢ BB Gestión", "$ Temas", "e @ OCR", "e Panel"
          text = text.replace(/^(\s*)((?:[^\w\s]\s*|[a-zA-Z]{1,2}\b\s*)+)(?=[A-ZÁÉÍÓÚÑ0-9])/g, (match, spaces, prefix) => {
            const trimmed = prefix.trim().toLowerCase();
            
            // Si es un guion, asterisco o viñeta normal, lo estandarizamos
            if (/^[\-\*•]$/.test(trimmed)) return spaces + '- ';
            
            // Excepciones explícitas que no queremos reemplazar
            if (/^(pd\.|c\+\+|c#|i\/o|[a-z]\+|[a-z]-)$/.test(trimmed)) return match;
            
            const hasSymbols = /[^\w\s]/.test(trimmed);
            const words = trimmed.split(/[^\w]+/).filter(w => w);
            
            // Lista de palabras cortas comunes que pueden iniciar una oración válidamente
            const validStarts = ['el', 'la', 'un', 'en', 'de', 'se', 'su', 'mi', 'tu', 'al', 'lo', 'si', 'no', 'ya', 'yo', 'te', 'os', 'me', 'a', 'y', 'o'];
            
            if (!hasSymbols) {
              // Si no hay símbolos, solo reemplazamos si contiene alguna "palabra" que no tiene sentido al inicio
              const allValid = words.every(w => validStarts.includes(w));
              if (allValid) return match; 
            } else {
               // Si tiene símbolos, pero son letras válidas con puntuación normal (ej. "Y, ", "El: ")
               if (words.length > 0 && words.every(w => validStarts.includes(w)) && /^[\w\s,;:]+$/.test(trimmed)) {
                   return match;
               }
            }
            
            // Si llegamos aquí, es basura de OCR o una viñeta extraña. Lo convertimos en viñeta limpia.
            return spaces + '- ';
          });
...
function cleanAndFormatWords(words) {
  ...
  for (let w = 0; w < words.length; w++) {
    ...
    // Falsos positivos: caracteres sueltos raros con baja confianza (%, @, #, ^, *, etc.)
    if (conf < 55 && /^[%@\^\*&\$#§~`]+$/.test(text)) continue;
    // Caracteres únicos sin sentido con muy baja confianza
    if (conf < 40 && text.length === 1 && /[^a-záéíóúüñA-ZÁÉÍÓÚÜÑ0-9]/.test(text)) continue;
